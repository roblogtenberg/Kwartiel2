package opdracht3V.impl;

import java.util.*;
import java.util.stream.Collectors;

import opdracht3V.interfaces.Trie;
...
public class Node<T> implements Trie<T> {
	 ...
	 * 
	 * @param nodeName
	 *            is used to give each child node a unique name
	...
	private List<Node<T>> getSortedChildren() {
		return children.values().stream().sorted((Node<T> c1, Node<T> c2) -> c1.getName().compareTo(c2.getName())).collect(Collectors.toList());
	}

	/**
	 * Collapses combines of single nodes into a single node. So [root]-[a]-[b]-[c]-[d] and [root]-[a]-[bcd] will both collapse into [root]-[abcd]. Collapsing stops at root, or when a node with
	 * multiple children is encountered
	 */
	...
	private Optional<Node<T>> getLeafStartingWithLetter(String firstLetter) {
		...
		return sortedChildren.stream().filter(child -> child.getName().startsWith(firstLetter) && child.isLeaf()).findFirst();
	}
	...
	protected int getNrOfValueNodes() {
		int nrOfValueNodesInChildren = children.values().stream().mapToInt(Node::getNrOfValueNodes).sum();
